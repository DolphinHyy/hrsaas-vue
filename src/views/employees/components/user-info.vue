      ...
      <el-row class="inline-info">
        <el-col :span="12">
          <el-form-item label="员工头像">
            <!-- 放置上传图片 -->
            <UploadImg ref="uploadImg" :on-success="onSuccess" />
          </el-form-item>
        </el-col>
      </el-row>
      <!-- 保存个人信息 -->
...
import { getStaffBassByIdAPI, getStaffInfoAPI, putStaffInfoAPI } from '@/api/staff'
...
export default {
  ...
  methods: {
    async getStaffInfo() {
      this.userInfo = await getStaffBassByIdAPI(this.userId)
      this.formData = await getStaffInfoAPI(this.userId)
      this.$refs.uploadImg.fileList = [{ name: this.userInfo.username, url: this.userInfo.staffPhoto }]
      // console.log(this.formData, this.userInfo)
    },
    async saveUser() {
    ...
    onSuccess(val) {
      this.userInfo.staffPhoto = val.url
    },
    savePersonal() {
